...
DrawRectangle(loaded_bitmap *Buffer, v2 vMin, v2 vMax, real32 R, real32 G, real32 B, real32 A = 1.0f)
{
	...
	// R, G, B vars represent the percentage of each color
...
inline void
DrawRectangleOutline(loaded_bitmap *Buffer, v2 vMin, v2 vMax, v3 Color, real32 Radius = 2.0f)
{
    // Math in here revolves around the fact we draw the outline slightly outside of the true bounds
...
DrawBitmap(loaded_bitmap *Buffer, loaded_bitmap *Bitmap, real32 RealX, real32 RealY, real32 CAlpha = 1.0f)
{
	...
	uint8 *SourceRow = (uint8 *)Bitmap->Memory + SourceOffsetY*Bitmap->Pitch + BITMAP_BYTES_PER_PIXEL*SourceOffsetX; // Starts at bottom
		...
		++Y)
	{
			...
			++X)
		{
            ...
            real32 A = 255.0f*(RSA + RDA - RSA*RDA); // Premultiplied alpha blend! Hard (day 83)
			real32 R = InvRSA*DR + SR; // Linear alpha blending formula (Day 38)
...
GetRenderEntityBasisPos(render_group *RenderGroup, render_entity_basis *EntityBasis, v2 ScreenCenter)
{
    ...
    v2 Center = {EntityGroundPointX + EntityBasis->Offset.x, EntityGroundPointY +  EntityBasis->Offset.y + EntityBasis->EntityZC*EntityZ};
...
#define PushRenderElement(Group, type) (type *)PushRenderElement_(Group, sizeof(type), RenderGroupEntryType_##type) // Magic stubbing, see PushPiece for insight
// Double pound is the paste operator
